...
interface URArm {
    ...
    /**
     * Sets the active tcp offset, i.e. the transformation from the output flange coordinate system to the
     * TCP as a pose.
     *
    ...
    /**
     * Move circular to position (circular in tool-space).
     * TCP moves on the circular arc segment from current joint position, through [positionVia] to [positionTo].
     * Accelerates to and moves with constant toolspeed [v]. Use the mode parameter to define the
     * orientation interpolation.
     *
     ...
     * @param r blend radius (m)
     * @param mode defines the orientation interpolation mode:
     * - `0`: Unconstrained mode. Interpolate orientation from current joint position to target joint position ([positionTo]) <br><br>
